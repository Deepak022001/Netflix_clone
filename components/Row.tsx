import { Movie } from '@/typing';
import Thumbnail from './thumbnail';
import { ChevronLeftIcon, ChevronRightIcon } from '@heroicons/react/24/solid';
import { useState, useRef } from 'react';
interface Props {
  title: string;
  movies: Movie[];
}
export const Row = ({ title, movies }: Props) => {
  const rowRef = useRef(null);
  const [isMoved, setisMoved] = useState(false);
  return (
    <div className='h-40 space-y-0.5 md:space-y-2'>
      <h2 className='w-56 cursor-pointer text-sm font-semibold text-[#e5e5e5] transition duration-200 hover:text-white md:text-2xl'>
        {title}
      </h2>
      <div className='group w-[100%] m-20 relative md:ml-2'>
        <ChevronLeftIcon className='absolute top-0 bottom-0 left-2 z-40 m-auto h-9 w-9 cursor-pointer opacity-0 transition hover:scale-125 group-hover:opacity-100' />
        <div
          ref={rowRef}
          className='transition duration-250 flex hover:scale-125 cursor-zoom-out items-center space-x-0.25 overflow-x-scroll md:space-x-2.5 md:p-2'
        >
          {movies.map((movie) => {
            return (
              <Thumbnail
                key={movie.id}
                movie={movie}
              />
            );
          })}
        </div>
        <ChevronRightIcon className='absolute top-0 bottom-0 left-2 z-40 m-auto h-9 w-9 cursor-pointer opacity-0 transition hover:scale-125 group-hover:opacity-100:' />
      </div>
    </div>
  );
};
